...
parser.add_argument("--original", "-a", help="Also save raw ACLs.", action="store_true")
parser.add_argument(
    "--wcu", "-w", help="Calculate WCU value of each rule.", action="store_true"
)
...
def parse_statement(obj):
    ...
    if first_key_obj not in ("SqliMatchStatement", "XssMatchStatement"):
        ...
        if first_key_obj == "ByteMatchStatement":
            ...
            field_match = (
                first_key(obj["ByteMatchStatement"]["FieldToMatch"])
                if first_key(obj["ByteMatchStatement"]["FieldToMatch"])
                != "SingleHeader"
            ...
            if "TextTransformations" in obj["ByteMatchStatement"]:
                wcu_statement["field"] = field_to_match(
                    obj["ByteMatchStatement"]["FieldToMatch"]
                )
...
def parse_rule(obj):
    ...
    return {"text": text.strip(), "wcu_rule": wcu_rule}
    ...
    for rule in input_acl:
        ...
        if "RuleGroupReferenceStatement" in rule["Statement"]:
            ...
            rulegroup = client.get_rule_group(
                Name=rule["Name"], Scope=scope, Id=rulegroup_id, ARN=rulegroup_arn
            )
            ...
            if len(statements) >= 1:
                ...
                for statement in statements:
                    ...
                    if rule_statement in ("ipset", "sql", "xss"):
                        ...
                        rule_wcu = (
                            rule_wcu
                            + wcu_dict[rule_statement]
                            + (
                                wcu_dict[rule_statement]
                                * out_rule[rule["Name"]].count(",")
                            )
                        )

                    if "text_transform" in statement:
                        text_transform_key = (
                            statement["text_transform"] + statement["field"]
                        )
...
for web_acl in web_acls:
    ...
    if save_original:
        with open(
            path.join(dir_path, "original", f'{web_acl["Name"]}-original-{TODAY}.json'),
            "w",
        ) as f:
