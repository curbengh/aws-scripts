...
parser.add_argument('--remediate', '-e',
...
parser.add_argument('--output', '-o',
  default = '',
  help = 'Output directory of CSV/XLSX.')
...
def paginator(name, value, operation_name, profile_name, region_name):
  ...
  ec_session = boto3.session.Session(profile_name = profile_name, region_name = region_name)
  ec_client = ec_session.client('ec2')
  ec2_response = ec_client.get_paginator(operation_name).paginate(
    Filters = [
      {
        'Name': name,
        'Values': [
...
for compliance_dict in compliance_list:
  ...
  if compliance == 'NON_COMPLIANT' and (account_id in arg_accounts or profile in arg_accounts):
    verbose_print(f'Querying ENIs attached to {sg_id}')

    session = boto3.session.Session(profile_name = profile, region_name = region)
    client = session.client('ec2')
    response = client.get_paginator('describe_network_interfaces').paginate(
      Filters = [
        {
          'Name': 'group-id',
          'Values': [
...
if len(non_compliant_sg) >= 1:
  for sg in non_compliant_sg:
    ...
    if remediate_level >= 1 and (account_id in arg_accounts or profile in arg_accounts):
      verbose_print(f'Fixing {sg_id}...')

      sg['FIXED'] = False
      # Query description of the security group
      default_sg = paginator('group-id', sg_id, 'describe_security_groups', profile, region)[0]['SecurityGroups'][0]

      # Query inbound/outbound rules of the security group
      describe_security_group_rules = paginator('group-id', sg_id, 'describe_security_group_rules', profile, region)
  ...
  out_csv = path.join(dir_path, f'non-compliant-vpc-default-security-group-closed-eni-attachments-{TODAY}.csv')
  out_xlsx = path.join(dir_path, f'non-compliant-vpc-default-security-group-closed-eni-attachments-{TODAY}.xlsx')

  # prevent overwriting previous report
  if len(arg_accounts) < len(list(ACCOUNT_NAME_DICT)):
    out_csv = path.join(dir_path, f'non-compliant-vpc-default-security-group-closed-eni-attachments-{NOW}.csv')
    out_xlsx = path.join(dir_path, f'non-compliant-vpc-default-security-group-closed-eni-attachments-{NOW}.xlsx')
  ...
  print("All default security groups are compliant to 'vpc-default-security-group-closed'")
